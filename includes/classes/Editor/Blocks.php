...
namespace Quasar\Editor;

use Quasar\Contracts\Bootable;
...
class Blocks implements Bootable {
	...
	// Should all Quasar registered blocks be included without having to list them.
	...
	public function allowed_blocks( array|bool $block_editor_context, \WP_Block_Editor_Context $editor_context ): array|bool {
		...
		if ( self::ENABLE_ALL_PULSAR_BLOCKS ) {
			...
			$quasar_blocks = array_filter(
				$block_names,
				function ( $key ) {
					return strpos( $key, 'quasar/' ) !== false;
				},
			);

			// Merge the quasar_blocks array with the allowed-blocks array
			return array_merge( Config::get( 'allowed-blocks' ) ?? [], $quasar_blocks );
		}
